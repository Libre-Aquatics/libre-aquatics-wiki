...
const articles = defineCollection({
  ...
  schema: z.object({
    ...
    // What the page describes, for the TechArticle `about` structured data
    // (src/lib/entity.ts). Left unset, it follows the section: vendors are
    // Organizations, equipment with a Manufacturer row is a Product, software
    // is a SoftwareApplication. Set `none` on a page about something else,
    // such as a file format.
    schemaType: z.enum(['Organization', 'Product', 'SoftwareApplication', 'none']).optional(),
    // Authoritative pages about the same subject (its Wikipedia article, its
    // Wikidata item), emitted as schema.org `sameAs` on the subject entity.
