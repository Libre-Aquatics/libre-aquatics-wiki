...
import { IMAGE_CREDITS } from '../src/data/image-credits.mjs';
...
for (const file of htmlFiles(distDir)) {
  ...
  for (const img of body.matchAll(/<img\b[^>]*>/gi)) {
    ...
    if (attr(img[0], 'alt') === undefined) {
      errors.push(`${route}: <img src="${src}"> has no alt attribute`);
    }
    // Photographs need an author and licence for the ImageObject metadata;
    // the logo is the site's own mark and is not a photograph.
    if (/^\/assets\/[^/]+\.(jpe?g|png|webp|avif|gif)$/i.test(src) && !IMAGE_CREDITS[src]) {
      errors.push(`${route}: ${src} has no entry in src/data/image-credits.mjs`);
    }
  }
